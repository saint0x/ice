...
- ✅ Release packaging verification now exists through `npm run verify:release`, which builds the packaged [Ice.app](/Users/deepsaint/Desktop/ice/src-tauri/target/release/bundle/macos/Ice.app) bundle and verifies its executable and bundle metadata.
...
- ✅ Codex thread history reads now pass both `projectId` and `threadId`, matching the backend’s stricter ownership validation so the frontend cannot cross-read another project’s thread history by thread id alone.
- ✅ Codex turns now run through a backend-scoped project boundary, so the frontend’s active-project thread actions map to one validated project root instead of a soft `projectId` tag only.
